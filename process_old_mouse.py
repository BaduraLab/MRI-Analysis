...
for iMousePath, MousePath in enumerate(mouse_path_list):
    ...
    os.system('flirt -in ' + mouse_path + ' \
                     ...
                     -inweight ' + mask_masked_path + ' \
                     -verbose 0')
    mouse_masked_flirted_image = nib.load(mouse_masked_flirted_path)
    mouse_masked_flirted = mouse_masked_flirted_image.get_fdata()

    # Apply flirt to mask, use mask on flirted image
    print('apply affine FLIRT to mask')
    os.system('flirt -in ' + mask_masked_path + ' \
                     -ref ' + reference_template_path + ' \
                     -out ' + mask_masked_flirted_path + ' \
                     -init ' + mouse_masked_flirt_path + ' \
                     -applyxfm \
                     -interp nearestneighbour \
                     -verbose 1')
    mask_masked_flirted_image = nib.load(mask_masked_flirted_path)
    mask_masked_flirted = mask_masked_flirted_image.get_fdata()
    mask_masked_flirted = mask_masked_flirted > 0
    mask_masked_flirted = mask_masked_flirted / np.max(mouse_masked_flirted)
    mouse_masked_flirted = mouse_masked_flirted * mask_masked_flirted
