...
#~ image="${folder_working}/mouse_mri_data/Pax5_R31Q_mutant/KO_2.nii" #~ input?
image="${folder_working}/KO_2_test.nii" #~ input?
image_dir="${image%.*}"
mkdir -p $image_dir
image_name="${image##*/}"
...
#~ bet $image my_betted_image.nii

#~ echo $image_warpaffine

echo flirt
flirt 	-in $image_reference \
		-ref $image \
		-omat "${image_warpaffine}_direct" \
		-verbose 1
...
fnirt 	--config=AMBMC_config.cnf \
		--in=$image_reference \
		--ref=$image \
		--cout="${image_warpcoef}_direct" \
		--aff="${image_warpaffine}_direct" \
		--verbose
		#~ --subsamp=4,4,2,2,2,2 \
#~ --iout=$image_warped \
#~ --miter=1,1,1,1,1,1 \
...
for i in ${!image_structures[@]}; do
	applywarp 	--ref=$image \
				...
				--warp="${image_warpcoef}_direct" \
				--out="${image_structures_invwarped[i]}_direct" \
				--interp=nn \
				--verbose
done
