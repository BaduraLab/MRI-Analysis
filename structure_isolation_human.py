...
# group_ids_list = [list(structure_list[0]['VolumeInteger']), list(structure_list[1]['VolumeInteger'])]
# group_name_list = ['ci', 'si']



group_ids_list = [[855]]
group_name_list = ['mask']
...
# to change inputs change annotation_path_list_list and template_path

# For each subject create cerebellum isolated files for both inmasked template and annotation
# transform_level_list = ['flirtedRigid', 'flirted', 'synned']
...
transform_level_list = ['native', 'flirtedRigid', 'flirted', 'synned', 'native-adjusted']
for transform_level in transform_level_list:

########################################################################################################################
    if transform_level == 'native':
        # annotation_path_list_list = [glob.glob(os.path.join(data_path, '*', '*orsuit_thrarg.nii.gz')),  ###
        #                              glob.glob(os.path.join(data_path, '*', '*subcortical_thrarg.nii.gz'))]  ###
        annotation_path_list_list = [glob.glob(os.path.join(data_path, '*', '*mask_thrarg.nii.gz'))]  ###
    elif transform_level == 'flirtedRigid':
        # annotation_path_list_list = [
        #     glob.glob(os.path.join(data_path, '*', 'retransform', '*orsuit_thrarg_' + transform_level + '.nii.gz')),
        #     glob.glob(os.path.join(data_path, '*', 'retransform', '*subcortical_thrarg_' + transform_level + '.nii.gz'))]
        annotation_path_list_list = [glob.glob(os.path.join(data_path, '*', 'retransform', '*mask_thrarg_' + transform_level + '.nii.gz'))]  ###
        ...
        # annotation_path_list_list = [glob.glob(os.path.join(data_path, '*', '*orsuit_thrarg_adjusted_lobular.nii.gz')),
        #                              glob.glob(os.path.join(data_path, '*', '*subcortical_thrarg.nii.gz'))]
        annotation_path_list_list = [glob.glob(os.path.join(data_path, '*', '*mask_thrarg.nii.gz'))]  ###
########################################################################################################################
    ...
    for iGroup in range(len(annotation_path_list_list)):
        ...
        for iSubject in range(len(annotation_path_list)):
            annotation_path = annotation_path_list[iSubject]
            subject = annotation_path.split(os.sep)[-1].split('_')[0]

            if transform_level in ['flirtedRigid', 'flirted', 'synned']:
                template_path = os.path.join(os.path.split(annotation_path)[0],
                                             annotation_path.split(os.sep)[-1].split('_')[0] + '_' + transform_level + '.nii.gz')###
            else:
                template_path = os.path.join(os.path.join(data_path, subject, subject + '_reoriented.nii.gz'))  ###

            Path_iso = os.path.join(data_path, subject, 'Isolations_' + transform_level)
            ...
            annotation_structure = annotation * annotation_in_structure
            template_structure = np.squeeze(template) * np.squeeze(annotation_in_structure)
            ...
            for iID in range(len(group_ids_list[iGroup])):
                ...
                if isinstance(iID_id_custom, list):
                    iID_name = 'SN'
                elif iID_id_custom == 855: # root id for now
                    iID_name = 'root'
                else:
                    iID_name = structure_list[iGroup].loc[structure_list[iGroup]['VolumeInteger'] == iID_id_custom, 'name'].iloc[0]
                # iID_acronym = structure_list[iGroup].loc[structure_list[iGroup]['VolumeInteger'] == iID_id_custom, 'acronym'].iloc[0]
                ...
                if iID_name == 'root':
                    annotation_in_ID = annotation > 0
                else:
                    annotation_in_ID = np.isin(annotation, iID_id_custom)

                annotation_ID = annotation * annotation_in_ID
                template_ID = np.squeeze(template) * np.squeeze(annotation_in_ID)
