...
for iInputPath, InputPath in enumerate(input_path_list):
    ...
    for iAnnotationPath, AnnotationPath in enumerate(annotation_path_list):
        AnnotationPath = annotation_path_list
        annotation_image = nib.load(AnnotationPath)
        annotation = annotation_image.get_fdata()

        annotation_invsynned = mapping.transform_inverse(annotation, interpolation='nearest') # nearest also works for non-probabilistic atlases!
        ...
        annotation_invsynned_path = InputPath.split('.')[0]+'_flirted_annotation_'+annotation_name+'.nii.gz'
        annotation_invsynned_invflirted_path = InputPath.split('.')[0]+'_annotation_'+annotation_name+'.nii.gz'
