...
reference_annotation_path_list = [os.path.join(reference_path, 'allen', 'annotation_full_custom_reoriented.nii.gz'), # not used
...
reference_structure_path_list = [os.path.join(reference_path, 'allen', 'voxel_count_custom.csv'),
                        os.path.join(reference_path, 'suit', 'atlasesSUIT', 'Lobules-SUIT_mc.csv'),
...
reference_structure_list = [pd.read_csv(reference_structure_path_list[0]),
                            ...
                            pd.read_csv(reference_structure_path_list[2])]
# id_list_list = [[1], [7]]
# id_list_list = [list(reference_structure_list[0]['VolumeInteger']), list(reference_structure_list[1]['VolumeInteger'])]
# reference_annotation_name_list = ['orsuit',
#                                   'subcortical']



reference_template_path_list = [os.path.join(reference_path, 'allen', 'mni_icbm152_t1_tal_nlin_asym_09b_hires_brain_reoriented.nii.gz'),
                                os.path.join(reference_path, 'suit', 'templates', 'SUIT.nii'),
                                os.path.join(reference_path, 'subcortical', 'CIT168_T1w_700um_reoriented.nii.gz')]
id_list_list = [[1],
                list(reference_structure_list[1]['VolumeInteger']),
                list(reference_structure_list[2]['VolumeInteger'])]
reference_annotation_name_list = ['all',
                                  'orsuit',
                                  'subcortical']
...
def arr4D_genotypeDifference(arr4D, genotype_list, reference_annotation_in_group):
    ...
    genotype_WT_logical = np.array([genotype == 'patient' for genotype in genotype_list])
    ...
    return arr4D_meanWT, arr4D_meanKO, arr4D_CohenD


for iGroup in range(len(id_list_list)):
    ...
    for id in id_list:
        if reference_annotation_name_list[iGroup] == 'all':
            structure_name = 'all'
        else:
            structure_name = reference_structure.loc[reference_structure['VolumeInteger'] == id, 'name'].iloc[0]
        ...
        if reference_annotation_name_list[iGroup] != 'all':
            ...
            reference_template_zeropadded, reference_template_crop_index = zeroPadImage(reference_template_group, reference_annotation_in_group, zeroPadRatio_adjusted)
            ...
            reference_template_zeropadded_image = save_image(reference_template_zeropadded, reference_template_image, reference_template_zeropadded_path)
            ...
            reference_annotation_zeropadded, reference_annotation_crop_index = zeroPadImage(reference_annotation_group,
                                                                                            reference_annotation_in_group,
                                                                                            zeroPadRatio_adjusted)
            reference_annotation_zeropadded_in_group = np.isin(reference_annotation_zeropadded, id)
        else:
        ...
        for iDataPath, DataPath in enumerate(data_path_list):
            ...
            # BUG: caudate nucleus human structure name contains space which flirt can't handle!
            ...
            jacobian_det_flirtAffine = computeJacobianDet(defField_flirtAffine,
                                                          reference_annotation_zeropadded_in_group)
            ...
            print(f'JD processing time for {subject} processing {structure_name} = {datetime.datetime.now() - start_time_JD}')
        ...
        defField_Jdet_flirtRigid_CohenD = \
        ...
        defField_Jdet_flirtAffine_CohenD = \
        ...
        defField_Jdet_syn_CohenD = \
            arr4D_genotypeDifference(defField_Jdet_syn, genotype_list, reference_annotation_zeropadded_in_group)

        # Save flirtRigid
        defField_magnitude_meanWT_path = os.path.join(SD_path, 'SD_flirtRigid_defField_Jdet_meanControl_' + structure_name + '.nii.gz')
        defField_magnitude_meanKO_path = os.path.join(SD_path, 'SD_flirtRigid_defField_Jdet_Patient_' + structure_name + '.nii.gz')
        defField_magnitude_CohenD_path = os.path.join(SD_path, 'SD_flirtRigid_defField_Jdet_CohenD_' + structure_name + '.nii.gz')
        ...
        # Save flirt
        defField_magnitude_meanWT_path = os.path.join(SD_path, 'SD_flirtAffine_defField_Jdet_meanControl_' + structure_name + '.nii.gz')
        defField_magnitude_meanKO_path = os.path.join(SD_path, 'SD_flirtAffine_defField_Jdet_Patient_' + structure_name + '.nii.gz')
        defField_magnitude_CohenD_path = os.path.join(SD_path, 'SD_flirtAffine_defField_Jdet_CohenD_' + structure_name + '.nii.gz')
        ...
        # Save syn
        defField_magnitude_meanWT_path = os.path.join(SD_path, 'SD_syn_defField_magnitude_meanControl_' + structure_name + '.nii.gz')
        defField_magnitude_meanKO_path = os.path.join(SD_path, 'SD_syn_defField_magnitude_Patient_' + structure_name + '.nii.gz')
        defField_magnitude_CohenD_path = os.path.join(SD_path, 'SD_syn_defField_magnitude_CohenD_' + structure_name + '.nii.gz')
        ...
        print(f'Processing time for {structure_name} = {datetime.datetime.now() - start_time_id}')
