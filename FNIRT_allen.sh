...
# Create Allen mask (flirted)
fslmaths $allen_annotation_to_AMBMC_flirted -thr 0.5 -bin $allen_annotation_bin_to_AMBMC_flirted
...
#~ flirt 	-in $allen_annotation \
		#~ -ref $AMBMC_average_template \
		#~ -out "${allen_annotation_to_AMBMC_flirted}_tri" \
		#~ -init $allen_to_AMBMC_flirt \
		#~ -applyxfm \
		#~ -verbose 1 \
		#~ -interp trilinear
...
#~ flirt 	-in $allen_annotation \
		#~ -ref $AMBMC_average_template \
		#~ -out "${allen_annotation_to_AMBMC_flirted}_sinc" \
		#~ -init $allen_to_AMBMC_flirt \
		#~ -applyxfm \
		#~ -verbose 1 \
		#~ -interp sinc
